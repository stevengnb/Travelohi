...
function HotelSearch() {
  ...
  const queryToBackend = debounce(async () => {
    if (search.trim() === "") {
      setResults({ cities: [], countries: [], hotels: [] });
      setRecent([]);
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/search?term=${search}`,
        {
          method: "GET",
          headers: {
  ...
  const addRecentSearch = async (searchTerm: string) => {
    ...
    if (!user) {
      setLoading(false);
      navigate(`/search-hotel?query=${encodeURIComponent(search)}`);
      return;
    }
      ...
      <div className={styles.contentFields}>
        <div className={styles.container}>
          <div className={styles2.inField}>
            <p className={styles.fieldT}>City, destination, or hotel name</p>
            <input
              type="text"
              name="search"
              placeholder="Search"
            ...
            inputActive && (
              <div className={styles.searchResult}>
                ...
                {results.hotels.map((hotel, index) => (
                  ...
                  >
                    <p>{hotel}</p>
                    <div className={styles.srType}>Hotel</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className={styles2.inField}>
          <p className={styles.fieldT}>Check-In</p>
          <input
            type="date"
            name="checkin"
